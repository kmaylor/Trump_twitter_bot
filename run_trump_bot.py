...
import random
from numpy import arange, where, isin, mean, array, exp, sqrt, dot, pi, diag, concatenate, histogram
...
def fit_pdf(data,bins,pdf,expected):
    counts,bounds = histogram(data,bins=bins,normed=True)
...
while True:
    ...
    if today!=str(datetime.now(timezone('US/Eastern')))[:10]:
        ...
        dt_params = fit_pdf([i for v in dt.values() for i in v],100,gauss,(10,4))
        ...
        while num_of_tweets <=0:
            num_of_tweets=int(round(normal(dt_params[0],dt_params[1])))
        num_of_tweets
        ...
        time_params = fit_pdf(times,arange(0,24.25,.25),bimodal,(5,1,.1,15,5))
        ...
        for i in arange(num_of_tweets):
            t=25
            while (t<0 or t>24):
                if uniform(0,1) >= time_params[2]:
                    t = normal(time_params[3],time_params[4])
                else:
                    t = normal(time_params[0],time_params[1])
                t+=7
                if t>24: t-=24
            times.append(t)
        print(times)
        ...
        del trump_tweets, dt, dt_params, time_params, num_of_tweets, hold, a, i, status

        for t in sorted(times):
            while True:
                if (t > convert_time(str(datetime.now(timezone('US/Eastern')))[11:19])-1/60) and \
                    (t < convert_time(str(datetime.now(timezone('US/Eastern')))[11:19])+1/60):
                    gen_tweet=['.','.','.','.','.']
                    while gen_tweet.count('.')>4:
                        try:
                            ...
                            gen_tweet==['.','.','.','.','.']
                    print(gen_tweet,t,convert_time(str(datetime.now(timezone('US/Eastern')))[11:19]))
                    #api.update_status(gen_tweet)
                    break
                else:
                    time.sleep((convert_time(str(datetime.now(timezone('US/Eastern')))[11:19])-t)*60**2)
        today = str(datetime.now(timezone('US/Eastern')))[:10]

    else:
        time.sleep((24-(convert_time(str(datetime.now(timezone('US/Eastern')))[11:19])))*60**2)
